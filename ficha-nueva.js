...
(function () {
  ...
  /* El texto es el MISMO, no se le quita ni se le agrega una palabra: lo unico
     ...
     De un tiron eran doce lineas grises seguidas y nadie las leia en el
     celular; partido en dos, la primera frase engancha y el resto acompaña. */
  var descHtml = '';
  if (p.desc) {
    var t = String(p.desc).trim();
    var corte = t.indexOf('. ');
    /* solo se parte si la primera frase da para entrada (ni muy corta ni el
       parrafo entero); si no, se deja de una pieza como estaba */
    if (corte > 40 && corte < t.length - 60) {
      descHtml = '<p class="entrada">' + esc(t.slice(0, corte + 1)) + '</p>'
               + '<p>' + esc(t.slice(corte + 2)) + '</p>';
    } else {
      descHtml = '<p>' + esc(t) + '</p>';
    }
  }
  ...
  function bloqueVideo() {
    ...
    return '<section class="bloque vid-wrap" style="position:relative;overflow:hidden;padding:36px 0;margin:0">'
      /* El video NO se baja al abrir la pagina: pesaba 2,1 MB de los 2,3 MB
         que cargaba la ficha. Se pone la fuente cuando entra en pantalla. */
